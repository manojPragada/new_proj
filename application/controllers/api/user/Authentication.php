...
class Authentication extends Api_controller {
    ...
    public function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = $this->my_model->get_data_row("customers", ["phone_number" => $phone_number]);
            if (!empty($data)) {
                ...
                if ($data->status == 0) {
                    ...
                    $user_data = $this->check_customer($data->access_token, "id,access_token,user_id,name,phone_number");
                    $arr = [
                        "status" => "valid",
                        "type" => "",
